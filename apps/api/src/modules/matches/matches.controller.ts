...
export async function correctMatchResultHandler(req: Request, res: Response): Promise<void> {
  const match = await matchesService.correctMatchResult(
    req.user!,
    req.params.id as string,
    req.body as CorrectMatchResultInput,
  );
  res.status(200).json({ match });
}
